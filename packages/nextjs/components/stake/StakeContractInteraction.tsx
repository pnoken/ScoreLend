...
export const StakeContractInteraction = ({ address }: { address?: string }) => {
  ...
  const { data: ExternalContract } = useDeployedContractInfo("ExternalContract");
  const { data: AnimaPOP } = useDeployedContractInfo("AnimaPOP");
  const { balance: stakerContractBalance } = useAccountBalance(StakerContract?.address);
  const { balance: externalContractBalance } = useAccountBalance(ExternalContract?.address);
  ...
  return (
    <div className="flex items-center flex-col flex-grow w-full bg-gray-100 px-4 gap-12">
      <div className="flex flex-col w-full border-opacity-50">
        <div className="grid card bg-white rounded-lg p-4 mt-4 shadow-md">
          <div className="flex justify-between">
            <h3 className="font-semibold">Annual Staking Rewards</h3>
            <h3 className="font-semibold">{apy ? `${(formatEther(apy) * 100).toFixed(2)}%` : "N/A"}</h3>
          </div>

          <div className="flex justify-between">
            <h3 className="font-semibold">Total MATIC Staked</h3>
            <h3>Master Token: {tokenID?.toString()}</h3>
            <div className="flex space-x-2">
              <ETHToPrice value={stakerContractBalance?.toString()} />
              <h3 className="text-sm mt-1">${stakerContractBalance && (nativeCurrencyPrice * stakerContractBalance).toFixed(3)}</h3>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* <div className="flex flex-col items-center bg-white shadow-lg border rounded-xl p-6 w-full md:w-2/3">
        ...
        <div className="grid card bg-white rounded-lg w-full p-4 mt-4 shadow-md">
        ...
        <div className="grid card bg-gray-100 w-full rounded-lg p-4 m-4 shadow-md">
          <div className="flex justify-between">
            <div className="flex gap-2">
              <h3>Staking Limit</h3>
              <div className="tooltip" data-tip="The maximum number of MATIC that can be staked at the moment">
            ...
            <div className="flex">
              <h3>{stakerContractBalance && threshold ? (formatEther(threshold) - formatEther(stakerContractBalance)).toFixed(2) : 0}</h3>
              <h3>/</h3>
              <h3>{threshold ? formatEther(threshold) : 0}</h3>
            </div>
          </div>
          <progress className="progress progress-secondary w-56" value={stakerContractBalance && threshold ? (formatEther(threshold) - formatEther(stakerContractBalance)) : 0} max={threshold ? formatEther(threshold) : 0}></progress>
        </div>
        ...
        {activeTab === 'stake' ? (
          ...
          <button disabled className="btn btn-primary w-full" onClick={() => stakeETH()}>
            Unstake
          </button>
        ) : (
          <button disabled={!isOpenForWithdraw} className="btn btn-primary w-full" onClick={() => withdrawETH()}>
            Withdraw
          </button>
        )}
      </div> */}
